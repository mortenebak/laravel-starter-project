...
<th {{$attributes->merge(['class' => 'px-6 py-3 bg-gray-100 text-left'])->only('class')  }}
>
    @unless($sortable)
        <span class="text-left text-xs leading-4 font-medium text-slate-700 uppercase tracking-wider">{{ $slot  }}</span>
    @else
        <button
